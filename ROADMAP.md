...
**T-16 — `ActionStatus`/`CommandStatus` proto enums default to OK (zero-value footgun)** ✅ interim done
...
Fixed (interim): `tests/unit/test_proto.rs` (`action_response_and_command_ack_literals_set_status_explicitly`) walks all tracked `.rs` files, finds every `ActionResponse {`/`KernelCommandAck {` struct literal, fails if any omits an explicit `status:` field — catches the zero-value-default footgun at test time without touching the wire format. The actual enum renumber (`ACTION_OK`/`COMMAND_OK` off zero, `*_UNKNOWN = 0` added) stays deferred to the next wire-breaking protocol version bump, since it invalidates every plugin built against current enum values across all three SDKs.
