...
|---|---|---|---|
| TLS (rustls) | encrypts the network path; self-signed auto-cert in `<private dir>/vyn-tls/`; local clients pin the cert; `tls: false` explicit opt-out | **SHIPPED** | D-07 |
...
- **Open audit items** (tracked in `AUDIT.md`/`ROADMAP.md`): S4
  ...
  fuzz harness (deferred), MA-01..MA-19 (2026-08-20 maintainability), and
   DC-1..DC-5 (dumb-core). S2 (events DB in per-user private dir — was `/tmp/vyn`, fixed PR #35) and S3
