...
| Item | Ref | Notes |
|------|-----|-------|
| ~~Zero-copy hot path (`Arc<[u8]>` payloads; drop per-frame/per-subscriber clones)~~ ✓ | AUDIT M-03-adj | `Frame.payload` is now `Arc<[u8]>` (was `Vec<u8>`). `write_frame_raw`'s common-path clone, `forward`/`broadcast`'s per-recipient clone (`src/ipc/protocol.rs`), and `EventBus::deliver`'s per-subscriber `build_frame` (`src/events/bus.rs`, previously `payload.to_vec()` inside the fan-out loop) are now refcount bumps, not byte copies — a broadcast/event-publish to N subscribers shares one buffer instead of allocating N. `sdk/rust` shares the same `Frame` type (depends on the kernel crate directly) and was updated in lockstep. `cargo test --all --all-features` (266 passing), `cargo clippy --all-targets --all-features -D warnings`, `cargo fmt --check` all clean on both the kernel crate and `sdk/rust`. |
...
| ~~Deterministic system `event_id` breaks re-delivery within retention~~ ✓ | AUDIT §7 | `sys-joined-{plugin_id}` (`ipc/protocol.rs`) and `sys-left-{plugin_id}` (`kernel/orchestrator.rs`) now suffix a millis timestamp — repeated joins/leaves of the same plugin_id within the SQLite retention window no longer collide on the `event_id` primary key and get silently no-op'd by `EventStore::persist`'s idempotent insert. `sys-died-{plugin_id}-{restart_count}` (`supervisor.rs`) already had a differentiator and was untouched. |
...
| ~~Dependency refresh: axum 0.8, nix, rusqlite, opentelemetry stack~~ ✓ | AUDIT §6 | `axum`/`axum-server` 0.7→0.8 (route path syntax `:id`→`{id}` in `src/api/server.rs`; `Message::Binary` now takes `Bytes` in `src/api/websocket.rs`), `nix` 0.28→0.31 (no call-site changes needed), `rusqlite` 0.31→0.40 (no call-site changes needed), `opentelemetry`/`opentelemetry_sdk`/`opentelemetry-otlp` 0.24/0.24/0.17→0.32/0.32/0.32 + `tracing-opentelemetry` 0.25→0.33 (`new_exporter()`/`new_pipeline()` builder API removed upstream; `src/utils/logging.rs::setup_otel_tracer` rewritten against `SpanExporter::builder().with_tonic()` + `SdkTracerProvider::builder().with_batch_exporter()`). `cargo test --all --all-features` (178 passing), `cargo clippy --all-targets --all-features -D warnings`, `cargo fmt --check` all clean; `sdk/rust` builds standalone with `--all-features`. |
| ~~SIGHUP reload + SIGTERM-ignoring-plugin shutdown tests~~ ✓ | AUDIT §4 | SIGTERM-ignoring-plugin shutdown was already covered (`graceful_shutdown_kills_each_plugin_on_its_own_deadline`, T-13). SIGHUP reload had zero test coverage: extracted the inline reload logic out of `Kernel::run`'s signal-handling closure into a standalone `reload_config_on_sighup()` (`src/kernel/orchestrator.rs`) and added 4 tests — 3 unit-level (no-path no-op, successful reload, malformed-YAML error surfaces) plus one true end-to-end test that installs the real SIGHUP handler, `raise()`s an actual signal at the process, and confirms the reload fires. |
